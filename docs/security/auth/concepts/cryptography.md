...
generally have a unique _private key_ for each combination of (client, identity provider).
...
??? example "Private JWKS (click to expand)"
...
with an identity provider. As part of the automated provisioning process, we will pre-register corresponding _public_
keys for your client at the identity provider. The identity provider will in turn then use these public keys to verify
the signature of your client assertions as proof of your client's identity.
...
??? example "Public JWKS (click to expand)"
