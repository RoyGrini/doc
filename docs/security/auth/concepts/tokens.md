...
A token are generally used as a [_Bearer_ token](https://datatracker.ietf.org/doc/html/rfc6750). Most resource servers
...
Access tokens are often in the form of [JWTs](#jwt) as it's generally quite convenient to use a single format.
...
as the refresh token is valid. Thus, it should generally be stored securely and used cautiously.

You should generally expire a session if:
...
Validation generally consists of the following steps:
...
- `aud` (**audience**)
    ...
    - The value of this is usually a unique identifier such as a client ID that belongs to either
