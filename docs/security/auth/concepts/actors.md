...
While we use the term identity provider, you might come across other terms such as _authorization server_ (AS) or
_OpenID provider_ (OP) which will mostly refer to the same thing.
...
A provider that only supports OAuth 2.0 (such as TokenX or Maskinporten) will generally only use
the `/oauth-authorization-server` suffix:

- `https://as.example.com/.well-known/oauth-authorization-server`

The platform will generally provide these URLs for you at runtime. Most libraries and frameworks support auto-discovery
of other relevant endpoints based on this document. We'll take a look at some essential endpoints below.
...
The `issuer` property defines the identifier for the provider, a URL that uses the "https" scheme and without query or
fragment components. Any [JWT](tokens.md#jwt) issued by the provider must have a `iss` claim with a value that is
exactly equal to the value found in the `issuer` property of the metadata document. This should
be [validated](tokens.md#token-validation) by [OpenID Connect clients](#client) and
...
Note that the tokens usually has an expiry time (often indicated by the `expires_in` field in the token response) and is
thus suitable for caching if network latency or throughput is a concern.
...
In cases where a client encounters tokens that are signed with a new key, libraries will usually just force a refresh of
their locally cached keys to find the matching key. See also
...
In our case, resource servers should require that requests to sensitive endpoints is authenticated with
...
As the platform does rotate these credentials relatively regularly, it shouldn't pose as a significant security risk to
use the `client_secret_post` method. However, we generally do recommend that [client assertions](#client-assertion) are
...
your client's own [private key](cryptography.md#private-keys). The JWT assertion will generally look something like
...
All [identity providers](#identity-provider) we use supports this method of authentication, also known as
the `private_key_jwt` method. This authentication method is also mandatory for all our clients across all the providers
with the exception of Azure AD.

The main differences between an assertion and a [client secret](#client-secret) is that the client's private key is
never exposed or sent as part of a request, and that the assertion itself is generally only valid for a short duration.

The provider only needs a public key (which the platform takes care of generating and registering) in order to verify
the assertion when receiving an authenticated request from the client. This method of authentication is thus considered
more secure than using client secrets.
